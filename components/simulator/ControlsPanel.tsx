...
export default function ControlsPanel({ selectedId, onSelect }: ControlsPanelProps) {
  ...
  const filtered = useMemo(() => {
    ...
    return ANATOMY_MODELS.filter(m => {
      const matchSearch = !q
        || tl(m.title, lc).toLowerCase().includes(q)
        || m.parts.some(p => tl(p, lc).toLowerCase().includes(q))
  ...
  return (
    <div className='flex flex-col gap-4'>
      ...
      <div className='px-3 py-2.5 rounded-xl bg-surface-light border border-border'>
        <p className='text-[10px] text-text-secondary uppercase tracking-wider mb-0.5'>{t('simulator.currentModel')}</p>
        <p className='text-sm font-semibold text-text-primary truncate'>{current ? tl(current.title, lc) : '—'}</p>
      </div>
        ...
        {ANATOMY_CATEGORIES.map(cat => (
          <button
            ...
            className={`px-2.5 py-1 text-xs font-medium rounded-lg transition-all duration-200 ${
              category === cat.en ? 'bg-primary text-white' : 'bg-surface-light text-text-secondary hover:text-text-primary'
            }`}
          >
            {tl(cat, lc)}
          </button>
      ...
      <div className='flex flex-col gap-1.5 pb-4'>
        <p className='text-[10px] font-semibold text-text-secondary uppercase tracking-wider'>
          {search ? `${t('simulator.results')} (${filtered.length})` : `${t('simulator.models')} (${ANATOMY_MODELS.length})`}
        </p>

        {filtered.length === 0 ? (
          <div className='text-center py-6 text-text-secondary'>
            <Search className='w-7 h-7 mx-auto mb-2 opacity-30' />
            <p className='text-sm'>&quot;{search}&quot; {t('simulator.notFound')}</p>
          </div>
        ) : (
          filtered.map(model => {
